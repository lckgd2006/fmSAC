...
% Optimized version for calculating only central row data - Struct parameter version
clc; clear; close all;
addpath(genpath('PSF'));
addpath(genpath('CSV'));
...
params.c = 3e10;             % Speed of light, measured in cm
params.lambda_s = 532e-7;    % Excitation wavelength, measured in cm
params.lambda_d = 488e-7;    % Competition wavelength, measured in cm
...
params.m_d = 1.0;            % Competition modulation contrast
params.m_s_1 = 0.3;          % Excitation light 1 modulation contrast
params.m_s_2 = 0.6;          % Excitation light 2 modulation contrast
params.m_s_3 = 0.9;          % Excitation light 3 modulation contrast
params.m_s_4 = 0.1;          % Excitation light 4 modulation contrast
...
%% Extract only central row data
center_row = ceil(LL/2);  % 第251行
fprintf('Calculating only central row data (Row %d)...\n', center_row);

% Extract central row data
I_exc_center = I_exc(center_row, :);
I_hexc_center = I_hexc(center_row, :);
num_points = length(I_exc_center);

%% Vectorized calculation of central row data
fprintf('Starting calculation of central row data (%d points)...\n', num_points);
...
% Calculate frequency component ratio (vectorized)
...
%% Remove side lobes (only for central row)
...
fprintf('  Excitation wavelength: %.1f nm, Competition wavelength: %.1f nm\n', params.lambda_s*1e7, params.lambda_d*1e7);
fprintf('  Excitation light intensity: %.1f kW/cm², Competition light intensity: %.1f kW/cm²\n', params.I_s/1e3, params.I_d/1e3);
fprintf('Modulation Parameters:\n');
fprintf('  Competition modulation contrast m_d: %.1f\n', params.m_d);
fprintf('  Excitation 1 modulation contrast m_s: %.1f (fmSAC_1)\n', params.m_s_1);
fprintf('  Excitation 2 modulation contrast m_s: %.1f (fmSAC_2)\n', params.m_s_2);
fprintf('  Excitation 3 modulation contrast m_s: %.1f (fmSAC_3)\n', params.m_s_3);
fprintf('  Competition 1 FWHM FWHM: %.1f nm (FWHM_1)\n', FWHM_1);
fprintf('  Competition 2 FWHM FWHM: %.1f nm (FWHM_2)\n', FWHM_2);
fprintf('  Competition 3 FWHM FWHM: %.1f nm (FWHM_3)\n', FWHM_3);
fprintf('  Traditional fmSAC FWHM FWHM: %.1f nm (FWHM)\n', FWHM);
fprintf('Frequency Parameters:\n');
fprintf('  f1: %.1f kHz, f2: %.1f kHz\n', params.f1/1e3, params.f2/1e3);
fprintf('Calculation Optimization:\n');
fprintf(['  Number of calculation points reduced from %d to %d, speedup ' ...
