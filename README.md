...
|Single-Point fmSAC Result Analysis and Parameter Exploration|`freqSAC_analysis.m`|Performs basic analysis on the frequency-domain modulation saturated absorption competition (fmSAC) experimental/simulation results of a single detection point, outputting core characteristic data at the single-point level.|
...
|Modulation Contrast and Imaging Optimization|`freqSAC_ContrastModulation_1.m`|Investigates the excitation light modulation contrast, analyzes its influence mechanism on the overall fmSAC imaging effect, and clarifies the correlation between contrast and imaging quality.|
||`freqSAC_plus_cm1.m`|Based on the analysis results of `freqSAC_ContrastModulation_1`, optimizes fmSAC imaging performance by reducing the excitation light modulation contrast.|
...
1. First run `freqSAC_nonlinear.m` to understand the nonlinear mechanism of fmSAC, laying a theoretical foundation for subsequent analysis.

2. For single-point analysis, prioritize running `freqSAC_analysis.m`, then explore parameter influences via `freqSAC_Impact.m`/`freqSAC_Impact_lambda.m`.

3. Imaging optimization codes (e.g., `freqSAC_plus_cm1.m`/`freqSAC_plus_cm2.m`) require prior execution of the corresponding contrast analysis codes (e.g., `freqSAC_ContrastModulation_1.m`).
...
5. The photobleaching GUI analysis (`freqSAC_bleaching_GUI.m`) can be run directly, with imaging results viewed by selecting the scan number through the interface.

## References

1. Wang W, Li C, Zhan Z, Zhang Z, Han Y, Kuang C, et al. Dual-modulation difference stimulated emission depletion microscopy to suppress the background signal. Adv Photonics. 2022;4:046001. https://doi.org/10.1117/1.AP.4.4.046001.
